...
function makeApp(videos, opts){
  ...
  return { doc, els, store, captured, fsCalls, requests, keydown: doc.listeners.keydown || [] };
}
...
// фокус у полі пошуку -> панель з'являється
appC.els.search.focus();
assert.ok(isOpen(), "фокус у пошуку має показати chips");
// клік теж
appC.els.chips.className = "";
appC.els.search.click();
assert.ok(isOpen(), "клік у пошуку має показати chips");
console.log("ok фокус/клік у пошуку відкриває chips");
...
appC.els.search.focus();
appC.els.search.listeners.keydown[0]({ key:"Escape", preventDefault(){} });
...
// перший chip збігається з автопошуком на старті
const defaultQuery = /var DEFAULT_QUERY = "([^"]+)"/.exec(html)[1];
assert.strictEqual(defaultQuery, chipList[0],
  "автопошук на старті має збігатись з першою підказкою");
...
assert.match(reqUrl, /[?&]q=africa%20online/, "у запиті має бути закодований q");
